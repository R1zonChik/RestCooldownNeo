...
public class CancelCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player)) {
            ...
            // Для команды отмены телепортации - просто вызываем метод без дополнительных сообщений
            if (label.equalsIgnoreCase("neocommand_cancel")) {
                // Метод cancelAllCommands уже сам отправляет сообщение
                ...
                if (!player.hasPermission("neocooldown.admin")) {
                    ...
                    if (args.length == 0) {
                        ...
                        if (args.length != 3) {
                            ...
                            newCommand = args[1];
                            String group = args[2]; // Здесь переменная group определена правильно
